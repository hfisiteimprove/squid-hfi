...
Http::Tunneler::handleException(const std::exception& e)
{
    ...
    connection->close();
    bailWith(new ErrorState(ERR_GATEWAY_FAILURE, Http::scInternalServerError, request.getRaw()));
}
...
Http::Tunneler::handleWrittenRequest(const CommIoCbParams &io)
{
    ...
    if (io.flag != Comm::OK) {
        const auto error = new ErrorState(ERR_WRITE_ERROR, Http::scBadGateway, request.getRaw());
...
Http::Tunneler::handleReadyRead(const CommIoCbParams &io)
{
    ...
    {
        const auto error = new ErrorState(ERR_READ_ERROR, Http::scBadGateway, request.getRaw());
