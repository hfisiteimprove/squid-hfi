...
ClientRequestContext::sslBumpAccessCheck()
{
    ...
    debugs(99, 5, HERE << "CsiteimproveC tunnelSslBumping: " << http->getConn()->port->flags.getTunnelSslBumping());

    if (http->request->method != Http::METHOD_CONNECT || http->redirect.status ||
            !Config.accessList.ssl_bump ||
            !http->getConn()->port->flags.getTunnelSslBumping()) {
        http->al->ssl.bumpMode = Ssl::bumpEnd; // SslBump does not apply; log -
        ...
        debugs(98, 5, HERE << "BsiteimproveB status: " << http->redirect.status
                            ...
                            << " tunnelSslBumping: "  << http->getConn()->port->flags.getTunnelSslBumping()  << " uri: " << http->uri );
        return false;
    }
...
ClientHttpRequest::processRequest()
{
    ...
    debugs(99, 4, "XsiteimproveX " << request->method << ' ' << uri << " Status: " <<
